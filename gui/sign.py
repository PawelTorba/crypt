...
class SignFrame(tk.Frame):
    def __init__(self, parent):
        ...
        self.file_label_output = tk.Label(self.sign_dialog_frame, text="Plik podpisu:")
        self.file_label_output.grid(row=2, column=0, padx=10, pady=10, sticky="w")
    ...
    def browse_output_file(self):
        #browse for output file to save signature
        self.output_file = filedialog.asksaveasfilename()
        if self.output_file:
            self.file_label_output.config(text=f"Plik wyjściowy: {self.output_file}")

    def browse_public_RSA_key(self):
        #browse for RSA public key file to verify signature
        self.rsa_key_file_public = filedialog.askopenfilename()
        if self.rsa_key_file_public:
            self.rsa_key_label.config(text=f"Klucz publiczny RSA odbiorcy: {self.rsa_key_file_public}")

    def browse_private_RSA_key(self):
        #browse for RSA private key file to sign
        self.private_rsa_key_file = filedialog.askopenfilename()
        if self.private_rsa_key_file:
            self.rsa_key_label.config(text=f"Klucz prywatny RSA nadawcy: {self.private_rsa_key_file}")
    ...
    def browse_signature_file(self):
        ...
        if self.signature_file:
            self.file_label_output.config(text=f"Plik podpisu: {self.signature_file}")
